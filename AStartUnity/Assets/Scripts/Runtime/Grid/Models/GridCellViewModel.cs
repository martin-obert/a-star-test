...
{
    /// <summary>
    /// Implements <see cref="IGridCellViewModel"/>
    /// </summary>
    public sealed class GridCellViewModel : IGridCellViewModel
    {
        ...
        public GridCellViewModel(GridCellDataModel dataModel, ITerrainVariant terrainVariant)
        {
            if (dataModel.TerrainType == TerrainType.Unknown)
                throw new ArgumentOutOfRangeException(nameof(dataModel.TerrainType), dataModel.TerrainType, "Terrain type not supported");
            
            var worldPosition = GridCellHelpers.ToWorldCoords(dataModel.RowIndex, dataModel.ColIndex);
            ColIndex = dataModel.ColIndex;
            RowIndex = dataModel.RowIndex;
